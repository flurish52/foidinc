    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click.self="closeModal">
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden">
            ...
            <div class="bg-primary text-white p-6">
                <h2 class="text-xl font-bold">Update Mass Intention Status</h2>
                <p class="text-primary-light mt-1">Update the status for {{ intention?.first_name }} {{ intention?.last_name }}</p>
...
const submitUpdate = () => {
    ...
    axios.patch('/update_intention/status', formData)
        .then(res=>{
            if (res.status === 200){
