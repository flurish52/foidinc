<template>
    <section class="py-6 max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex gap-3 items-center border p-4 rounded">
            ...
            <img v-if="newCard.preview" :src="newCard.preview" class="w-16 h-16 object-cover rounded" />

            <button @click="addCard" class="px-3 py-1 bg-indigo-600 text-white rounded">Add</button>
        </div>
        ...
        <div v-if="cards?.length" class="space-y-3">
            ...
            >
                <div class="flex items-center gap-4">
                    <img v-if="card.thumbnail" :src="'/storage/' + card.thumbnail" class="w-16 h-16 object-cover rounded" />
                    <div>
